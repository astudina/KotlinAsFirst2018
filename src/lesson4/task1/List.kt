...
fun convertToString(n: Int, base: Int): String {
    val convert1 = convert(n, base)
    val letters = "abcdefghijklmnopqrstuvwxyz"
    var result = ""
    for (element in convert1) {
        if (element < 10) {
            result += element
        } else {
...
fun roman(n: Int): String {
    var n1 = n
    var result = ""
    val noRoman = listOf(1000, 900, 500, 400, 100, 90, 50, 40, 10, 9, 5, 4, 1)
    val yesRoman = listOf("M", "CM", "D", "CD", "C", "XC", "L", "XL", "X", "IX", "V", "IV", "I")
    for (i in 0 until yesRoman.size) {
        while (n1 >= noRoman[i]) {
            result += yesRoman[i]
            n1 -= noRoman[i]
        }
